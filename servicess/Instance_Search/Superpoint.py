...
class SuperPointFrontend(object):
  ...
  def get_descriptor_from_keypoints(self, img, keypoints):
    assert img.ndim == 2, 'Image must be grayscale.'
    assert img.dtype == np.float32, 'Image must be float32.'
    H, W = img.shape[0], img.shape[1]
    inp = img.copy()
    ...
    pts = np.array([[kp[1], kp[0], 1] for kp in keypoints]).T
    # --- Process descriptor.
    D = coarse_desc.shape[1]
    if pts.shape[1] == 0:
      desc = np.zeros((D, 0))
  ...
  def get_custom_descriptor(img, keypoints, model, output_dim=128, cuda=False):
    """
    Tạo descriptor từ keypoints với kích thước cố định là output_dim (mặc định: 128).

    Arguments:
    img -- Ảnh đầu vào dưới dạng numpy array (HxW), dạng grayscale, giá trị float32.
    keypoints -- Danh sách các keypoint với kích thước (N, 2) tương ứng với tọa độ x, y.
    model -- Mô hình mạng dùng để tạo descriptor.
    output_dim -- Kích thước của descriptor mong muốn, mặc định là 128.
    cuda -- Nếu dùng GPU, đặt cuda=True.

    Returns:
    desc -- Descriptor kích thước N x output_dim
    """
    ...
    H, W = img.shape[0], img.shape[1]
    inp = img.copy().reshape(1, 1, H, W)
    inp = torch.from_numpy(inp)
    if cuda:
        inp = inp.cuda()

    # Forward pass của ảnh qua model để lấy đặc trưng thô
    with torch.no_grad():
        features = model(inp)[1]  # Ở đây [1] là coarse_desc

    # Khởi tạo desc với kích thước (N, output_dim)
    desc = np.zeros((len(keypoints), output_dim))

    # Chuyển đổi keypoints thành tensor, chuẩn hóa vị trí cho phù hợp với ảnh
    samp_pts = torch.from_numpy(np.array(keypoints)).float()
    ...
    # Sử dụng grid_sample để lấy mô tả từ các vị trí keypoint
    sampled_desc = torch.nn.functional.grid_sample(features, samp_pts)
    sampled_desc = sampled_desc.view(features.shape[1], -1).cpu().numpy().T

    # Đảm bảo mỗi vector descriptor có kích thước output_dim
    for i in range(len(desc)):
        desc[i] = sampled_desc[i][:output_dim]
